...
public class Main {
	public static void main() {
		SwingUtilities.invokeLater(() -> {
			try {
				...
				JOptionPane.showMessageDialog(null, "Fehler beim Starten des Spiels: " + e.getMessage(), "Fehler",
	...
	private static void handleGameEnd(ActionEvent event, GameState state, JFrame frame) {
		if (!state.status().isRunning()) {
			...
			JOptionPane.showMessageDialog(frame, switch (state.status()) {
			...
			}, "LockSnake", JOptionPane.INFORMATION_MESSAGE);
